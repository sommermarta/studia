---
title: "Pakiety dplyr i tidyr z Internetu"
...
auta2012 %>%
  summarise(min_cena = min(Cena.w.PLN),
            max_cena = max(Cena.w.PLN),
            max_przebieg = max(Przebieg.w.km, na.rm=TRUE))
```
...
auta2012 %>%
  ...
  summarise(mediana_przebiegu = median(Przebieg.w.km, na.rm=TRUE),
            mediana_ceny = median(Cena.w.PLN, na.rm=TRUE)) %>%
  mutate(iloraz = mediana_ceny/mediana_przebiegu) %>%
  arrange(iloraz) %>%
  "["(1,1)
...
spread(tsdtr210, geo, value) %>% filter(vehicle == "TRN")

tsdtr210 %>% filter(substr(geo,1,1) %in% c("E", "e")) %>% spread(geo, value) %>% filter(vehicle == "TRN")
...
waska <- iris %>% gather(wymiar, wartosc, -Species)
head(waska)
...
auta2012 %>%
  filter(Przebieg.w.km<50000, Marka=="Volkswagen", Model=="Golf", Wersja=="IV") %>%
