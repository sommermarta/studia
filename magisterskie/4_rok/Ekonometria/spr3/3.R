...
p2 <- read.csv2("C:\\Users\\Marta\\Desktop\\Marta\\studia\\rok4\\Ekonometria\\spr3\\portfele_z2.csv",sep=";",header=TRUE)
head(p2)
...
tabela1_rm <- t(matrix(rm1,nrow=4))
tabela2_rm <- t(matrix(rm2,nrow=4))
tabela3_rm <- t(matrix(rm3,nrow=4))

# tabele dla SMB:

tabela2_smb <- t(matrix(smb2,nrow=4))
tabela3_smb <- t(matrix(smb3,nrow=4))

# tabele dla HML:

tabela2_hml <- t(matrix(hml2,nrow=4))
tabela3_hml <- t(matrix(hml3,nrow=4))

# tabele dla WML:

tabela3_wml <- t(matrix(wml3,nrow=4))
